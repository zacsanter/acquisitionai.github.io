...
// Variable to hold the user's message
let message = '';

// Reference to the user input element
const userInput = document.getElementById('user-input');

// Update the 'message' variable whenever the user types something
userInput.addEventListener('input', (event) => {
  message = event.target.value.trim();
});

// Reference to the send icon
const sendIcon = document.getElementById('send-icon');

// Function to send the message
function sendMessage() {
  if (message) {
    // Logic to send the message, similar to what happens after pressing the "Enter" key
    // For example:
    // interact(message);
...
// Attach the sendMessage function to the icon's click event
