...
console.log("localStorage companyName:", localStorage.getItem('companyName'));
console.log("localStorage uniqueId:", localStorage.getItem('uniqueId'));

// Retrieve username and company name from localStorage with different variable names
const fetchedUsername = localStorage.getItem('username');
const fetchedCompanyName = localStorage.getItem('companyName');

if (!fetchedUsername || !fetchedCompanyName) {
    console.error("Username or Company Name not found in localStorage");
} else {
    ...
    const options = {
        ...
        body: JSON.stringify({username: fetchedUsername, companyname: fetchedCompanyName})
    };
    ...
    fetch('https://general-runtime.voiceflow.com/state/user/' + uniqueId + '/variables', options)
        .then(response => {
            if (!response.ok) {
                console.error("Voiceflow API Error:", response.statusText);
            }
            return response.json();
        })
        .then(response => {
            console.log("Voiceflow API Response:", response);
        })
        .catch(err => {
            console.error("Error in Voiceflow API call:", err);
        });
}
...
console.log("Username from localStorage:", fetchedUsername);
console.log("Company Name from localStorage:", fetchedCompanyName);
console.log("uniqueId:", uniqueId);
...
if (fetchedUsername && fetchedCompanyName) {
    const options = {
        ...
        body: JSON.stringify({username: fetchedUsername, companyname: fetchedCompanyName})  // Adjusted variable name
    };
