...
let customerId;    let companyName = null;
    let email = null;
    let firstName = null;
    let phone = null;
    ...
    const options = {
        ...
        headers: {
            ...
            Authorization: 'VF.DM.64dde9e69f9d270007f90d8e.L9W42WVd2SmrTHLH' // Consider securing this token
        },
        body: JSON.stringify({
  ...
  // Only call interact('#launch#', customerId) if there are no saved messages
  const customerId = updateUserVariables();  if (!savedMessages) {
    interact('#launch#', customerId)
  }
...
async function interact(input, customerId) {    const userMessageData = {
        uniqueId: customerId,
