...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const chat-window = document.getElementById('chat-window');
  // Load messages from local storage
  const savedMessages = localStorage.getItem('messages')
  if (savedMessages) {
    chat-window.innerHTML = savedMessages
  }
   if (localStorage.getItem('messages')) {
    chat-window.innerHTML = localStorage.getItem('messages');
...
input.addEventListener('keypress', (event) => {
  if (event.key === 'Enter') {
    ...
    if (userInput) {
      ...
      chat-window.appendChild(messageElement)

      // Save messages to local storage
      localStorage.setItem('messages', chat-window.innerHTML)
    ...
    chat-window.appendChild(typingIndicator)
    ...
    response.forEach((item) => {
        ...
        if (item.type === 'choice') {
            item.payload.buttons.forEach(button => {
                ...
                chat-window.appendChild(buttonElement);
            });
        }
    });
}
    ...
    if (response) {
      response.forEach((item) => {
        if (item.type === 'speak' || item.type === 'text') {
          ...
          chat-window.appendChild(imageElement)
        }
      })
    } else {
      console.info('Error')
...
function handleButtonClick(button) {
    ...
    chat-window.appendChild(userMessageElement);
...
function handleButtonClick(button) {
    // Ensure chat-window is accessible or define it if needed
    const chat-window = document.querySelector('.chat-window');  // Adjust selector as per your HTML
    ...
    chat-window.appendChild(userBtnMessageElement);
